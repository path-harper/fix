...
The app operates by receiving push webhooks from GitHub. When a push is made to a repository where the app is installed, GitHub sends a webhook payload to your deployed app endpoint. The app then generates an installation access token using the installation ID from the webhook, clones the repository using that token for authentication, checks out the affected branch, and (when needed) executes `git filter-branch` on the commits unique to that branch (relative to the repository default branch) to modify commit messages by removing " add ", " Add ", leading "add ", or leading "Add " phrases. Finally, it pushes the rewritten history back to the branch using `--force-with-lease`, skipping actions if the push originated from a bot to prevent infinite loops.
The app operates by receiving push webhooks from GitHub. When a push is made to a repository where the app is installed, GitHub sends a webhook payload to your deployed app endpoint. The app then generates an installation access token using the installation ID from the webhook, clones the repository using that token for authentication, checks out the affected branch, and identifies new commits relative to the repository's default branch. For these new commits, it modifies commit messages by removing " add ", " Add ", leading "add ", or leading "Add " phrases. The rewritten history is then pushed back to the branch using `--force-with-lease`, ensuring the default branch is never rewritten and skipping actions if the push originated from a bot to prevent infinite loops.
